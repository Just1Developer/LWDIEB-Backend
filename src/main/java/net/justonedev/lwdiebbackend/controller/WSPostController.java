...
public class WSPostController {
    ...
    public ResponseEntity<Map<String, Object>> post(@RequestBody Map<String, String> requestBody) {
        if (signature == null) signature = new Signature(SIGNATURE_SECRET);
        ...
        if (!serverPassword.equals(SERVER_PASSWORD) || !signature.equals(checksum) || userId.replaceAll("[0\\-]", "").isEmpty() && !command.endsWith("-global")) {
            String reason = !serverPassword.equals(SERVER_PASSWORD) ? "Incorrect server password" : !signature.equals(checksum) ? "Invalid signature" : "Invalid command: %s with userId: %s".formatted(command, userId);
        ...
        switch (command) {
            case "light":
            case "dark":
                socketSessionHandler.sendSocketMessageBuffered(userId, "themeUpdate", command, 2100);
                break;
            default:
                socketSessionHandler.sendSocketMessage(userId, command);
        }
