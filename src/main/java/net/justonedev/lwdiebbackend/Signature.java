...
public final class Signature {
    ...
    /**
     * Computes the signature for a given string and outputs it as HighBits#LowBits, as unsigned decimal values.
    ...
    public long sign(String data) {
        ...
        long second =
                generateHash(
                        data.substring(data.length() / 4, (int) Math.round(data.length() / 1.5)));
        long third =
                generateHash(data.substring(data.length() / 2, data.length() - data.length() / 4));
        long fourth =
                generateHash(
                        data.substring(data.length() / 3, data.length() - data.length() / 5 - 1));
    ...
    private long finalizeSignature(long signature) {
        ...
        if (secret.length() < 10) return signature * Long.parseLong(secret);
        int index = 0;
        while (index < secret.length()) {
            var nextIndex = index + Math.min(secret.length() - index, (int) Math.ceil((secret.charAt(index) - '0') / 2d) + 11); // Maximum number is 5, max digits is 17
